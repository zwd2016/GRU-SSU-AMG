...
import tensorflow as tf#provide tf.cond
...
class GRUCell(Layer):
    ...
    def __init__(self, units,
        ...
        self.select_ratio = select_ratio#2020-9-20
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            # previous and candidate state mixed by update gate raw hidden state update strategy
            h = z * h_tm1 + (1 - z) * hh
        if self.implementation == 2:#new hidden update strategy
            comparison = tf.less_equal(self.select_ratio * (z * h_tm1 + (1 - z) * hh),h_tm1)
            h = tf.where(comparison, z * h_tm1 + (1 - z) * hh, h_tm1)
        if 0 < self.dropout + self.recurrent_dropout:
            if training is None:
...
class SkipGRUCell(Layer):
    """Cell class for the SkipGRU layer.
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            #2020-2-12 skip-binaryity
            z_tilde = z + tf.minimum(x_z, 1. - z)
            #global update_gate #global variable to solve UnboundLocalError: local variable 'update_gate' referenced before assignment (in line-2024)
            update_gate = tf.round(z_tilde)

        #if self.implementation == 1:
        # previous and candidate state mixed by update gate 
        h = z * h_tm1 + (1 - z) * hh
        h = update_gate * h + (1. - update_gate) * hh #2020-2-12 skip-binaryity updating
...
class SkipGRU(RNN):
    ...
    def __init__(self, units,
        ...
        cell = SkipGRUCell(units,
                       ...
                       implementation=implementation,
                       reset_after=reset_after)
        super(SkipGRU, self).__init__(cell,
                                  return_sequences=return_sequences,
                                  return_state=return_state,
                                  go_backwards=go_backwards,
                                  stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._dropout_mask = None
        self.cell._recurrent_dropout_mask = None
        return super(SkipGRU, self).call(inputs,
                                     mask=mask,
                                     training=training,
                                     initial_state=initial_state)
...
class LSTMCell(Layer):
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 2: #T-LSTM modify
            if 0 < self.dropout < 1.:
                inputs_i = inputs * dp_mask[0]
                inputs_f = inputs * dp_mask[1]
                inputs_c = inputs * dp_mask[2]
        ...
        if self.implementation == 3:
            ...
            c = (1.0-K.tanh(f)) * c_tm1 + (1.0-K.tanh(i)) * self.activation((x_c) + K.dot(h_tm1_c,self.recurrent_kernel_c))#modify T-LSTM 
        ...
        if self.implementation == 4:
            ...
            c = f * c_tm1 + (K.tanh(i) + 1.0) * self.activation((x_c) + K.dot(h_tm1_c,self.recurrent_kernel_c))#modify-2020-1-10
...
class LSTMEFGCell(Layer):
    ...
    def __init__(self, units,
                 ...
                 implementation=1,
                 **kwargs):
        super(LSTMEFGCell, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            f = self.recurrent_activation(x_f + K.dot(h_tm1_f,
                                                      self.recurrent_kernel_f))
            c = f * c_tm1 + (1 - f) * K.softsign(x_c + K.dot(h_tm1_c,
                                                            self.recurrent_kernel_c))
            o = self.recurrent_activation(x_o + K.dot(h_tm1_o,
                                                      self.recurrent_kernel_o))
        ...
        if self.implementation == 3:
            ...
            c = (1.0-K.tanh(f)) * c_tm1 + (1.0-K.tanh(i)) * self.activation((x_c) + K.dot(h_tm1_c,self.recurrent_kernel_c))#modify T-LSTM 2019-7-26
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(LSTMEFGCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class LSTMEFG(RNN):
    ...
    def __init__(self, units,
        ...
        cell = LSTMEFGCell(units,
                        ...
                        recurrent_dropout=recurrent_dropout,
                        implementation=implementation)
        super(LSTMEFG, self).__init__(cell,
                                   return_sequences=return_sequences,
                                   return_state=return_state,
                                   go_backwards=go_backwards,
                                   stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._dropout_mask = None
        self.cell._recurrent_dropout_mask = None
        return super(LSTMEFG, self).call(inputs,
                                      mask=mask,
                                      training=training,
                                      initial_state=initial_state)
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(LSTMEFG, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
